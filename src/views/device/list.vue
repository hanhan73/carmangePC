  ...
  <div class="wrapper">
    <div class="m-b-5 ovf-hd fz-14 search-bar">
      ...
      <el-cascader
        ...
        change-on-select
      ></el-cascader>
      <el-input size="mini" clearable class="w-300" placeholder="请输入内容" v-model.trim="searchText" @change="handleSearchTextChange" @keyup.native.enter="getAll">
        <template slot="prepend">
          <el-select v-model="searchType" class="w-100" @change="handleSearchTypeChange">
      ...
      <el-select v-model="params.isActive"
       ...
       <el-select size="mini" v-model="params.typename" clearable placeholder="请选择设备类型">
        <el-option
      ...
      <!-- <el-radio-group size="mini" v-model="params.isActive" @change="getAll">
      ...
      <el-radio-group size="mini" v-model="params.status" @change="getAll">
        <el-radio-button :label="null">全部</el-radio-button>
        <el-radio-button :label="2">绑定</el-radio-button>
        <el-radio-button :label="1">未绑定</el-radio-button>
      </el-radio-group>
      <div class="m-l-5 p-r-5" style="border-left: 1px solid #ccc"></div>
      <el-radio-group size="mini" v-model="params.isFllow" @change="getAll">
        <el-radio-button :label="null">全部</el-radio-button>
        <el-radio-button :label="1"><i class="el-icon-star-off" style="padding: 0px"></i></el-radio-button>
        <el-radio-button :label="0"><i class="el-icon-star-on" style="padding: 0px"></i></el-radio-button>
      </el-radio-group>
      <el-popover
        ref="batchSearch"
        placement="bottom"
        ...
        <el-tabs tab-position="left" style="height: 220px;">
          ...
          <!-- <el-tab-pane label="顺序查询">
            <el-radio-group v-model="batchSearch.type" size="mini">
              <el-radio-button v-for="(i, index) in searchTypeOptions" :key="index" :label="i.label">{{i.text}}</el-radio-button>
            </el-radio-group>
            <el-form class="m-t-20" size="mini" label-width="135px" label-position="left">
              <el-form-item>
                <span slot="label" class="fz-12 dis-blo" style="text-indent: 15px">开始{{computedTxet}}</span>
                <el-input v-model="batchSearch.startText" placeholder="请输入内容"></el-input>
              </el-form-item>
              <el-form-item>
                <el-select slot="label" v-model="batchSearch.sequenceType">
                  <el-option :value="0" label="输出查询数量"></el-option>
                  <el-option :value="1" :label="`结束${computedTxet}`"></el-option>
                </el-select>
                <el-input v-model="batchSearch.endText" placeholder="请输入内容"></el-input>
              </el-form-item>
            </el-form>
            <el-alert
              :title="batchSearchTips[batchSearch.sequenceType]"
              type="info"
              show-icon
              :closable="false">
            </el-alert>
    ...
    <el-table
      ...
      <el-table-column
        min-width="260px"
        label="操作">
        <template slot-scope="{ row: { id, sn, vehicleid, partnerid, status, isFllow, carnumber }, $index, row }">
          <div class="pos-rel p-l-30">
            ...
            <el-button-group>
              ...
              <el-button type="primary" size="mini" @click="$refs.vehicleData.open(id, vehicleid, sn, partnerid)">编辑</el-button>
              <el-button v-if="isAdmin" type="danger" size="mini" @click="confirmDelete(id)">删除</el-button>
            </el-button-group>
          </div>
        </template>
      </el-table-column>
...
export default {
  ...
  beforeCreate() {
    this.zh_name = '设备';
    this.name = '/device/getDeviceList';
