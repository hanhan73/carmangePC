    ...
    <section v-else class="bindDeviceWrapper">
      ...
      <div class="right">
        <div class="search-bar">
          ...
          <el-button key="bind" size="mini" type="primary" @click="handleBatchBind">一键绑定</el-button>
        ...
        <el-table key="unbind"
          ...
          <el-table-column
            label="操作">
            <template slot-scope="{ row: { sn, isEdit, fenceId }, $index }">
              ...
              <div v-else>
                ...
                <el-button v-else key="save" size="mini" type="text" @click.stop="handleSaveEditMindType($index, fenceId)">保存</el-button>
                <el-button size="mini" type="text" @click.stop="handleRemoveRelation(sn, fenceId)">移除</el-button>
              </div>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    async add() {
      ...
      // const { sn } = this.tableData;
      // console.log(this.tableData);
      try {
        // let vIds = this.$refs.tree.getCheckedKeys();
        // vIds = vIds.filter(i => typeof i === 'string' &&
        // i.substring(0, 1) === 'n').map(i => parseInt(i.substring(2), 10));
        // if (!vIds.length) {
        //   this.isLoading = false;
        //   return _g.toastMsg('warning', '所选分组无设备在内');
        // }
        // const { data } = await this.apiPost('/device/qryDeviceDataByVIds', { vIds });
        this.form.sns = this.tableData.filter(i => i.sn !== '').map(i => i.sn);
    ...
    renderContent1(h, { node, data }) {
      return (
        ...
        >
          ...
          &nbsp;
          <i>
            (<i class="c-green f-w-b">{data.onlineNum}</i>/{data.totalNum})
          </i>
        </span>
      );
    },
    async open(coordinates, currentArea) {
      ...
      this.params.desc = this.currentArea.cityName;
      this.params.partnerid = Lockr.get('partnerid');
      this.getAll();
      this.dialogVisible = true;
      this.preLoading = true;
      const { data } = await this.apiPost('/partner/getPartnerDeviceTree', {});
      const treeData = this.normal(data);
    ...
    async handleSaveEditMindType($index, fenceid) {
      const { editMindType, sn } = this.tableData[$index];
      try {
        await this.apiPost('/fence/upRelation', { mindType: editMindType, sn, fenceId: fenceid });
    ...
    handleSingleBind(sns) {
      this.$emit('handleBindDevice', { fenceid: '0', sns, fencetype: '1', desc: this.form.desc, pois: this.form.pois });
    },
    handleBatchBind() {
      ...
      this.$emit('handleBindDevice', { fenceid: '0', sns: this.selection.map(i => i.sn).join(','), fencetype: '1', desc: this.form.desc, pois: this.form.pois });
    },
    handleRemoveRelation(sns, fenceid) {
      this.$confirm('确定移除该设备与围栏的绑定关系吗?', '提示', {
        ...
        try {
          await this.apiPost('/fence/delRelation', { fenceId: fenceid, sns });
    ...
    handleNodeClick(node) {
      this.pagination.curPage = 1;
      this.params.partnerid = node.partnerid;
      this.params.userId = node.userid;
