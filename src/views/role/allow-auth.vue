...
export default {
  ...
  methods: {
    ...
    async open(roleid) {
      ...
      const { data } = await this.apiPost('/role/getAllMenu', {});
      this.menu = this.normalizeMenu(data);
      const resource = await this.apiPost('/role/queryMyResoure', { roleid });
    ...
    async add() {
      ...
      this.isLoading = true;
      try {
        await this.apiPost('/role/addPower', { menuIds, roleId });
